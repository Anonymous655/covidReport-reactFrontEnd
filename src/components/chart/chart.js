...
export default class chart extends Component {
  ...
  getStateData = async () => {
    ...
    await data.data.statewise.map((list) => {
      if (list.state !== "Total") {
  ...
  // to randomly generate colors for each time it iterates
  ...
  render() {
    return (
      <div className="container main-div mt-4">
        <div>
          <h3 className="heading">Pie Chart For Statewise Confirmed Cases</h3>
        </div>
